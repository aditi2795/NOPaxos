...
#maxThreads = {"unreplicated": 25, "vr": 5, "batch": 25, "fastpaxos": 4,
#        "nopaxos": 20}
#stepSize = {"unreplicated": 3, "vr": 1, "batch": 3, "fastpaxos": 1, "nopaxos":
#        3}
threadRanges = {"unreplicated": range(1, 6, 1) + range(6, 25, 3), "vr": range(1,
    5), "batch": range(1, 25, 3), "fastpaxos": range(1, 4), "nopaxos": range(1,
        17,
        3)} 
for protocol in protocols:
    ...
    plt.plot(throughputList, latencyList, linestyle='-', marker='o', label=legend[protocol])
plt.legend()
plt.xlabel("Throughput (ops/sec)")
plt.xlim([0, 200])
plt.gca().xaxis.set_major_formatter(FormatStrFormatter('%dK'))
plt.ylabel("Latency (microsec)")
plt.title("NOPAXOS Figure 5 replication (5 replicas)")
plt.savefig('Figure5-5.png')
