...
for m in match_iter1:
    num_requests = m.group(1);
    total_requests += num_requests;
    num_seconds = m.group(2);
    if num_seconds > max_seconds:
        max_seconds = num_seconds;

if max_seconds != -1:
    print "Throughput (requests/sec): " + total_requests/max_seconds
else:
...
for m in match_iter2:
    avg_latency = m.group(2);
    sum_avg_latency += avg_latency;
    num_avg_latency += 1;

if num_avg_latency > 0:
    print "Avg latency: " + sum_avg_latency/num_avg_latency;
else:
